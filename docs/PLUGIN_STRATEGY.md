...
- **2b.1 — Close the local-agent contract. _Backend conformance implemented; capability UX next._** Separate local and strict ACP Registry v1 validation profiles are implemented: local entries may omit `distribution`, while registry entries must satisfy the constraints represented by the pinned v1 schema, including URI formats, non-null typed properties, nested-field refusal, binary cardinality, and lossless root-field round-tripping. Pinned schema plus real-entry fixtures live under `tests/fixtures/acp_registry/v1/`. The fake provider now crosses authenticated diagnostics and the real WebSocket session protocol with visible rejection/quarantine assertions. Next, project provider origin plus negotiated session configuration into provider-neutral DTOs. Frontend fallbacks remain necessary: `lib/providers.ts` already degrades gracefully for unknown ids, while `lib/provider-modes.ts` and `providers/model_validation.rs` still need catalog-driven behavior. Only after those surfaces are covered should a small installed-provider diagnostics/management screen call step 2b user-shippable.
