...
Status: proposal (rev. 2026-08-01). Grounded in a full audit of the service, desktop, and packaging layers, plus prior-art research (VS Code, Obsidian, Zed, JetBrains, Raycast, Figma) current to mid-2026.
...
- **2a — Registry dynamism (internal, zero behavior change).** `static ADAPTERS: &[(&str, &dyn AgentRuntimeAdapter)]` (`providers/mod.rs:21`) becomes first-party static slice + dynamic slice behind one accessor; six iteration sites migrate (`provider_catalog_entries_live_for_cwd`, `spawn_runtime_startup_warmups`, `notify_worktree_created_for_all_providers`, `resolve_effective_provider`, `merge_extra_models`, `runtime_adapter`). De-staticize the `&'static str` returns on ~6 trait methods (`default_permission_mode_wire`, `access_mode_setting_key`, `worktree_config_paths`, `post_plan_approval_*`) plus `RuntimeError::CliNotFound { provider }` → `Cow<'static, str>`/`String`.
- **2b — User-declared agents.** One data-driven generic ACP adapter is parameterized by a validated ACP Registry agent entry plus a Cadencr-owned installation record (source, resolved executable, host policy, enablement, and local overrides). A local descriptor may point at an existing executable, but the behavioral contract remains negotiated ACP. Do **not** add manifest capability booleans, model/mode lists, permission maps, or auth behavior: `initialize`, session configuration, and standard ACP events are authoritative. The agent appears with an ACP source badge and negotiated capability summary. Frontend fallbacks: `lib/providers.ts` `getProviderMetadata` already degrades gracefully for unknown ids; `lib/provider-modes.ts` (hardcoded per-provider mode table) and `providers/model_validation.rs` (`PROVIDER_ALIAS_METADATA`) need catalog-driven fallbacks. Headline value: run any compatible ACP agent inside the parts of Cadencr's workspace UX its capabilities support; rewind/fork and other non-standard features remain unavailable unless a contained adapter can provide them.
...
| **2 — Bring your own agent** | 2a registry dynamism + de-staticization → 2b generic ACP adapter + local agent entries → 2c discovery recipes + ACP Agent Registry alignment (M1 signing and M2 blocklist land before marketplace executables)              | L    |
