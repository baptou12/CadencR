...
- **Conversation status:** restore the historical sidebar meanings independently of branding: blue working (breathing dot) and unread (steady dot), amber permission shield, primary question glyph, muted idle dot. Pending indicators retain the existing interactive gate popover. The 8px live dots use a restrained 2px halo; reduced motion and disabled animations stop breathing. These sidebar-specific legacy colors are separate from the green ready/worktree status below.
- **Tokens:** `--sidebar`, `--sidebar-border`, `--accent` (hover bg), `--primary` (question/active), `--muted-foreground` (eyebrows, idle, mono marks), `--sidebar-foreground` (text + worktree-group wash). Sidebar working/unread retain Tailwind `blue-500`; permission retains `amber-400`.
