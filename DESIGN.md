...
- **Provider mark:** idle rows use the mono silhouette. A working agent keeps that silhouette and paints it with the old robot treatment (`text-blue-500` + pulse). A pending gate wraps the mark as the popover trigger and tints it amber (permission) or primary (question). Unread is a small `bg-blue-500` overlay on an idle mark. Status never occupies a second column.
- **Tokens:** `--sidebar`, `--sidebar-border`, `--accent` (hover bg), `--primary`/`--primary-glow` (active state), `--muted-foreground` (eyebrows + counts), `--acc-green`/`--acc-pink`/`--acc-orange` (project dots), `--sidebar-foreground` (text + the worktree-group wash). Working/unread on the mark use the same Tailwind `blue-500` as the retired robot glyph.
