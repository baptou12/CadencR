...
function buildStyle(bundle: AnserBundle): React.CSSProperties {
  ...
  const decs = bundle.decorations?.length ? bundle.decorations : bundle.decoration ? [bundle.decoration] : []
...
export function parseAnsi(text: string): React.ReactNode {
  ...
  const hasStyle = bundles.some(b => b.was_processed && (b.fg || b.bg || b.decoration || (b.decorations && b.decorations.length > 0)))
  if (!hasStyle) {
