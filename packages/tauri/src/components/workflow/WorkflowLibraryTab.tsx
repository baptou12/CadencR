  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-8">
      ...
      {isLoading ? (
        ...
        <>
          <LibrarySection title="Built-in Presets">
            {presets.length === 0 ? (
          ...
          <LibrarySection title="Custom Workflows">
            {custom.length === 0 ? (
              <p className="text-sm text-muted-foreground">No custom workflows yet. Create one or fork a preset.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {custom.map((def) => (
                  <CustomCard
...
function PresetCard({ definition, onFork, onView }: { definition: WorkflowDefinition; onFork: () => void; onView: () => void }) {
  return (
...
function CustomCard({ definition, onEdit, onFork, onDelete }: { definition: WorkflowDefinition; onEdit: () => void; onFork: () => void; onDelete: () => void }) {
  return (
    <div className="rounded-lg border border-border bg-card p-4 space-y-3">
      <div>
        <h3 className="font-medium">{definition.name}</h3>
