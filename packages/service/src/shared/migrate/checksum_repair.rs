...
use super::checksum_repair_data::{
    AGENT_MESSAGE_UUID_VERSION, FEATURE_STATUS_TRIGGERS, LEGACY_SETTING_KEYS,
    OLD_AGENT_MESSAGE_UUID_CHECKSUMS, OLD_REMOVE_WS_FEATURE_CHECKSUMS,
...
pub(super) async fn repair_known_sqlx_checksum_mismatches(
    ...
    repair_agent_message_uuid_checksum(pool, migrator).await?;
...
async fn repair_agent_message_uuid_checksum(
    pool: &SqlitePool,
    migrator: &sqlx::migrate::Migrator,
) -> anyhow::Result<()> {
    let Some((applied_checksum, success)) =
    ...
    verify_agent_message_uuid_base_postconditions(pool).await?;
...
async fn verify_agent_message_uuid_base_postconditions(pool: &SqlitePool) -> anyhow::Result<()> {
