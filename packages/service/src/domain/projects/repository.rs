...
mod tests {
    ...
    async fn test_delete_project_cascade() {
        ...
        // Create project
        let project = create_project(&pool, "Cascade", "/tmp/cascade").await.unwrap();
        let pid = project.id;
        ...
        // Create plan + phase
        let plan_id: i64 = sqlx::query_as::<_, (i64,)>(
            "INSERT INTO plans (feature_id) VALUES (?) RETURNING id"
        )
        .bind(fid)
        .fetch_one(&pool)
        .await
        ...
        let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM agent_sessions WHERE feature_id = ?")
            .bind(fid).fetch_one(&pool).await.unwrap();
        assert_eq!(count.0, 0);
        ...
        let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM project_settings WHERE project_id = ?")
            .bind(pid).fetch_one(&pool).await.unwrap();
        assert_eq!(count.0, 0);

        let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM feature_settings WHERE feature_id = ?")
            .bind(fid).fetch_one(&pool).await.unwrap();
        assert_eq!(count.0, 0);

        let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM diff_viewed_files WHERE feature_id = ?")
            .bind(fid).fetch_one(&pool).await.unwrap();
        assert_eq!(count.0, 0);
    }
    ...
    async fn test_get_project_settings() {
        let pool = setup_test_db().await;
        let project = create_project(&pool, "Settings", "/tmp/settings").await.unwrap();
        let pid = project.id;

        // Set a column-based setting
        set_project_setting(&pool, pid, "agent_autonomy", "full").await.unwrap();
        // Set a KV setting
        set_project_setting(&pool, pid, "custom_key", "custom_value").await.unwrap();
    ...
    async fn test_set_project_setting_column_vs_kv() {
        let pool = setup_test_db().await;
        let project = create_project(&pool, "ColVsKv", "/tmp/colvskv").await.unwrap();
        let pid = project.id;

        // Column-based: agent_autonomy, branch_prefix, qa_prompt, parallel_execution
        set_project_setting(&pool, pid, "agent_autonomy", "supervised").await.unwrap();
        set_project_setting(&pool, pid, "branch_prefix", "feat/").await.unwrap();

        let row: (Option<String>, Option<String>) = sqlx::query_as(
            "SELECT agent_autonomy, branch_prefix FROM projects WHERE id = ?"
        )
        .bind(pid)
        .fetch_one(&pool)
        .await
        ...
        set_project_setting(&pool, pid, "instructions", "do stuff").await.unwrap();
        set_project_setting(&pool, pid, "test_command", "cargo test").await.unwrap();

        let kv_count: (i64,) = sqlx::query_as(
    ...
    async fn test_get_project_model_settings() {
        let pool = setup_test_db().await;
        let project = create_project(&pool, "Models", "/tmp/models").await.unwrap();
        let pid = project.id;
    ...
    async fn test_set_project_model_setting() {
        let pool = setup_test_db().await;
        let project = create_project(&pool, "ModelSet", "/tmp/modelset").await.unwrap();
        let pid = project.id;

        set_project_model_setting(&pool, pid, "plan", "claude-3-haiku").await.unwrap();
        set_project_model_setting(&pool, pid, "execute", "claude-3-opus").await.unwrap();
        set_project_model_setting(&pool, pid, "qa", "claude-3-sonnet").await.unwrap();
