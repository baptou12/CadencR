...
use super::events_plan::map_plan;
use super::events_stream_blocks::{
    drain_streaming_block_stops, message_start_for, stream_delta_event, stream_start_event,
    stream_stop_event, EventIndexer,
};
use super::events_tool_call::{map_tool_call_start, MappedUpdate};
use super::events_tool_call_update::map_tool_call_update;
...
pub async fn mirror_session_info_update(body: &Value, current_model: &Arc<RwLock<Option<String>>>) {
    if let Some(model) = body.get("model").and_then(Value::as_str) {
        super::events_config_option::write_if_changed(current_model, Some(model.to_string())).await;
    }
