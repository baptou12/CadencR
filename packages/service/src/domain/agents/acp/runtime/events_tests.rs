//! Inline unit tests for `events.rs` — split out via `#[path]` so the parent
//! module stays under the project's 400-line file ceiling. The W3 task spec
//! explicitly authorises this split.

// Note: included via `#[path = "events_tests.rs"] mod tests;` from
// `events.rs`, so `super` here resolves to the `events` module. This means
// `mirror_session_info_update` / `session_update_to_events` (defined in the
// parent) come in via `super::*`, while sibling helpers live in
// `super::super::*` (the `runtime` module).
