//! Streaming-event constructors used by the OpenCode ACP path.
//!
//! Originally the OpenCode HTTP transport built every flavour of
//! `RuntimeEvent` here. With the HTTP transport gone, only the streaming
//! block helpers (`message_start`, `content_block_start/delta/stop`)
//! survive — they're still consumed by:
//!
//! - the per-child `StreamSynthesizer` in
//!   `acp/upstream_workaround/subagent_listener.rs` (sub-agent events
//!   discovered via the embedded HTTP backend), and
//! - the provider-neutral ACP runtime layer at `acp/runtime/events_*`,
//!   which imports them under `http_*` aliases (legacy naming —
//!   despite the alias they're transport-agnostic builders).
