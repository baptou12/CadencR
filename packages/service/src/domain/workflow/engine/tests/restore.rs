...
async fn test_restore_on_reconnect_populates_paused_sessions() {
    ...
    sqlx::query(
        "INSERT INTO agent_sessions (feature_id, agent_type, status, runtime_session_id) VALUES (1, 'plan', 'paused', 'cc-resume-123')"
    ...
    assert!(
        got_agent_paused,
        "should have sent agent_paused with runtime_session_id"
    );
}

#[tokio::test]
async fn test_restore_on_reconnect_ignores_sessions_without_runtime_session_id() {
...
async fn test_restore_on_reconnect_ignores_other_features() {
    ...
    sqlx::query(
        "INSERT INTO agent_sessions (feature_id, agent_type, status, runtime_session_id) VALUES (999, 'plan', 'paused', 'other-feature')"
    ).execute(&engine.write_pool).await.unwrap();
...
async fn test_restore_on_reconnect_restores_paused_queue_items_with_session() {
    ...
    sqlx::query(
        "INSERT INTO agent_sessions (id, feature_id, agent_type, status, runtime_session_id) VALUES (50, 1, 'execute', 'paused', 'cc-queue-456')"
    ).execute(&engine.write_pool).await.unwrap();
...
async fn test_restore_on_reconnect_deduplicates_by_slot() {
    ...
    sqlx::query(
        "INSERT INTO agent_sessions (id, feature_id, agent_type, status, runtime_session_id) VALUES (1, 1, 'plan', 'paused', 'old-session')"
    ).execute(&engine.write_pool).await.unwrap();
    sqlx::query(
        "INSERT INTO agent_sessions (id, feature_id, agent_type, status, runtime_session_id) VALUES (2, 1, 'plan', 'paused', 'new-session')"
    ).execute(&engine.write_pool).await.unwrap();
...
async fn test_restore_on_reconnect_restores_multiple_session_agents() {
    ...
    sqlx::query(
        "INSERT INTO agent_sessions (id, feature_id, agent_type, status, runtime_session_id) VALUES (10, 1, 'session', 'paused', 'cc-session-a')"
    ).execute(&engine.write_pool).await.unwrap();
    sqlx::query(
        "INSERT INTO agent_sessions (id, feature_id, agent_type, status, runtime_session_id) VALUES (11, 1, 'session', 'paused', 'cc-session-b')"
    ).execute(&engine.write_pool).await.unwrap();
...
async fn test_restore_on_reconnect_only_restores_paused_status() {
    ...
    sqlx::query(
        "INSERT INTO agent_sessions (id, feature_id, agent_type, status, runtime_session_id) VALUES (1, 1, 'plan', 'running', 'running-session')"
    ).execute(&engine.write_pool).await.unwrap();

    sqlx::query(
        "INSERT INTO agent_sessions (id, feature_id, agent_type, status, runtime_session_id) VALUES (2, 1, 'prd', 'paused', 'paused-session')"
    ).execute(&engine.write_pool).await.unwrap();

    engine.restore_on_reconnect().await.unwrap();

    // Running sessions with runtime_session_id are now recovered as paused for resume
    assert!(
        engine
            .agent_manager
            .paused_sessions
            .contains_key(&AgentSlot::Plan),
        "running session with runtime_session_id should be recovered as paused"
    );
    assert!(
        engine
            .agent_manager
