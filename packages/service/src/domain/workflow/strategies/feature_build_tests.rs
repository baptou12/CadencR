...
async fn test_enriched_execute_prompt_autonomy_1_shows_approval() {
    ...
    let prompt = super::feature_build_prompts::build_enriched_execute_prompt(&pool, &phase, 1, 0).await.unwrap();
...
async fn test_enriched_execute_prompt_autonomy_3_shows_auto_commit() {
    ...
    let phase: Phase = sqlx::query_as("SELECT * FROM phases WHERE id = 1")
        .fetch_one(&pool).await.unwrap();
    let prompt = super::feature_build_prompts::build_enriched_execute_prompt(&pool, &phase, 3, 0).await.unwrap();
