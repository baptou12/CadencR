    ...
    loop {
        ...
        for entry in ENGINES.iter() {
            ...
            if engine.active_items().is_empty() {
                ...
                let threshold = if engine.has_sender() { idle_threshold_secs } else { detached_idle_threshold_secs };
                if age > threshold {
...
pub async fn handle_workflow_action(
    ...
    match envelope.action.as_str() {
        ...
        "plan.approved" | "plan.rejected" => workflow_complex::handle_plan_approval(envelope, sender).await,
        "prd.approved" | "prd.rejected" => workflow_complex::handle_prd_approval(envelope, sender).await,
        "populate_queue" => workflow_complex::handle_populate_queue(envelope, sender, app_state).await,
        ...
        "retry_worktree_setup" => workflow_interact::handle_retry_worktree_setup(envelope, sender, app_state).await,
        "permission.respond" => workflow_interact::handle_permission_respond(envelope, sender, app_state).await,
...
macro_rules! engine_call {
    ($envelope:expr, $sender:expr, $payload_ty:ty, |$p:ident, $e:ident| $call:expr, $err_code:literal, $err_prefix:literal) => {{
        let Some(($p, $e)) = parse_and_get_engine::<$payload_ty>(&$envelope, $sender) else { return };
        if let Err(e) = $call {
            send_workflow_error($sender, &$envelope.id, $err_code, &format!(concat!($err_prefix, ": {e}"), e = e));
        }
    }};
}

async fn handle_skip_item(envelope: WsEnvelope, sender: &WsSender) {
    engine_call!(envelope, sender, WorkflowSkipItemPayload,
        |p, engine| engine.skip_item(p.item_id).await,
        "SKIP_FAILED", "Failed to skip item");
}

async fn handle_retry_item(envelope: WsEnvelope, sender: &WsSender) {
    engine_call!(envelope, sender, WorkflowRetryItemPayload,
        |p, engine| engine.retry_item(p.item_id).await,
        "RETRY_FAILED", "Failed to retry item");
}
...
async fn handle_set_autonomy(envelope: WsEnvelope, sender: &WsSender) {
    ...
    engine.autonomy_level().store(payload.level, Ordering::Relaxed);
    info!(feature_id = payload.feature_id, level = payload.level, "autonomy level updated");
...
async fn handle_set_parallel(envelope: WsEnvelope, sender: &WsSender, app_state: &AppState) {
    ...
    info!(feature_id = payload.feature_id, enabled = payload.enabled, max_parallel = max, "parallel execution updated");
