...
pub(super) async fn handle_permission_respond(envelope: WsEnvelope, sender: &WsSender, app_state: &AppState) {
...
async fn persist_qa_answer(
    ...
    let Some(ref updated_input) = payload.updated_input else { return };
    let Some(answers) = updated_input.get("answers") else { return };
    let Some(answer_text) = answers.get("0").and_then(|v| v.as_str()) else { return };
    let Some(db_session_id_ref) = engine.active_items().get(&payload.agent_slot) else { return };
...
pub(super) async fn handle_prompt_send(envelope: WsEnvelope, sender: &WsSender, app_state: &AppState) {
...
pub(super) async fn handle_interrupt(envelope: WsEnvelope, sender: &WsSender, app_state: &AppState) {
    ...
    if let Some(engine) = get_engine(payload.feature_id) {
        ...
        handle_interrupt_no_engine(&payload, &slot, sender, &envelope, app_state, interrupted_payload).await;
    }
}
...
async fn handle_interrupt_no_engine(
    ...
    match repo::get_queue_item(&app_state.read_pool, *item_id).await {
        Ok(Some(item)) if item.pid.is_some() => {
            handle_pid_interrupt(item.pid.unwrap(), *item_id, payload, slot, sender, envelope, app_state, &interrupted_payload).await;
        }
        Ok(Some(_)) => {
...
async fn handle_pid_interrupt(
    ...
    warn!(item_id, pid, "no-engine PID fallback interrupt (PID reuse risk)");
...
pub(super) async fn prepare_worktree(
    ...
    let worktree_path = worktree::ensure_worktree(
        &app_state.read_pool, &app_state.write_pool, feature_id, project_id, engine_sender,
    ).await?;
