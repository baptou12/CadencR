...
use super::workflow::{
    ensure_background_tasks, parse_and_get_engine, parse_payload, send_workflow_error, to_value,
    ENGINES,
};
...
pub(super) async fn handle_feature_start(
    ...
    let Some(payload) = parse_payload::<WorkflowFeatureStartPayload>(&envelope, sender) else {
        return;
    };
    let feature_id = payload.feature_id;
    let feature_row: Result<Option<(String,)>, _> =
        sqlx::query_as("SELECT type FROM features WHERE id = ?")
            .bind(feature_id)
    ...
    let feature_type = match feature_row {
        ...
        Err(e) => {
            send_workflow_error(
                ...
                &envelope.id,
                "LOOKUP_FAILED",
                &format!("Failed to look up feature: {e}"),
            );
            return;
        }
    };
    let is_workflow_feature = feature_type == "ws-feature";
    let workflow_type_str = payload
        .workflow_type
        .unwrap_or_else(|| "feature_build".to_string());
    let workflow_type =
        WorkflowType::from_str(&workflow_type_str).unwrap_or(WorkflowType::FeatureBuild);
    handle_engine_create_or_reattach(
        feature_id,
        is_workflow_feature,
        &workflow_type,
        sender,
    ...
    if let Some(engine) = ENGINES.get(&feature_id) {
        ...
        .await;
        let autonomy: u8 = autonomy_str.and_then(|s| s.parse().ok()).unwrap_or(1);

        engine
            .autonomy_level()
            .store(autonomy, std::sync::atomic::Ordering::Relaxed);
    ...
    let resp = WsEnvelope::reply(
        ...
        "workflow",
        "feature.started",
        to_value(WorkflowFeatureStartResponse {
            feature_id,
...
async fn handle_engine_create_or_reattach(
    ...
    if let Some(existing) = ENGINES.get(&feature_id) {
        info!(
            feature_id,
            "reattaching sender to existing engine on reconnect"
        );
...
pub(super) async fn handle_start_plan(
    envelope: WsEnvelope,
    sender: &WsSender,
    app_state: &AppState,
) {
    handle_start_agent_with_worktree::<WorkflowStartPlanPayload, _, _>(
        ...
        |p, engine| async move {
            engine
                .spawn_plan_agent(&p.description, &p.images.unwrap_or_default())
                .await
        },
    )
    .await;
...
pub(super) async fn handle_start_prd(
    envelope: WsEnvelope,
    sender: &WsSender,
    app_state: &AppState,
) {
...
async fn handle_approval(
    ...
    let Some((payload, engine)) =
        parse_and_get_engine::<WorkflowApprovalPayload>(&envelope, sender)
    else {
        return;
    };
    ...
    info!(
        feature_id,
        approved, kind, "resolving approval via permission channel"
    );
    ...
    match engine.respond_permission(slot.clone(), response).await {
        Ok(()) => info!(
            feature_id,
            kind, "approval routed through permission channel"
        ),
        Err(e) => {
    ...
    let ack = WsEnvelope::reply(
        &envelope.id,
        "workflow",
        format!("{kind}.approval_resolved"),
        to_value(WorkflowApprovalResolvedPayload {
...
async fn update_approval_status(engine: &WorkflowEngine, kind: &str, approved: bool) {
    if kind == "plan" {
        engine
            .set_status(if approved {
                WorkflowStatus::ReadyToBuild
            } else {
                WorkflowStatus::Planning
            })
            .await;
...
pub(super) async fn handle_populate_queue(
    envelope: WsEnvelope,
    sender: &WsSender,
    app_state: &AppState,
) {
    let Some((payload, engine)) =
        parse_and_get_engine::<WorkflowPopulateQueuePayload>(&envelope, sender)
    else {
        return;
    };
        ...
        .await
    {
        Ok(items) => {
            let ack = WsEnvelope::reply(
                ...
                "workflow",
                "queue.populated",
                to_value(WorkflowQueuePopulatedPayload {
                    feature_id,
        ...
        Err(e) => {
            send_workflow_error(
                ...
                &envelope.id,
                "POPULATE_FAILED",
                &format!("Failed to populate queue: {e}"),
            );
        }
    }
}

pub(super) async fn handle_start_build(
    envelope: WsEnvelope,
    sender: &WsSender,
    app_state: &AppState,
) {
    let Some((payload, engine)) =
        parse_and_get_engine::<WorkflowStartBuildPayload>(&envelope, sender)
    else {
        return;
    };
...
async fn auto_populate_if_empty(
    ...
    let item_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM workflow_queue WHERE feature_id = ?")
            .bind(payload.feature_id)
    ...
    if item_count == 0 {
        info!(
            feature_id = payload.feature_id,
            "start_build: queue empty, populating first"
        );
        let strategy = strategies::get_strategy(&engine.workflow_type).inspect_err(|e| {
            send_workflow_error(sender, &envelope.id, "STRATEGY_ERROR", e);
        })?;
...
async fn send_queue_state(feature_id: i64, sender: &WsSender, engine: &WorkflowEngine) {
    if let Ok(items) =
        crate::domain::features::repository::get_queue_for_feature(&engine.read_pool, feature_id)
            .await
    {
...
pub(super) async fn handle_continue(envelope: WsEnvelope, sender: &WsSender) {
    let Some((payload, engine)) =
        parse_and_get_engine::<WorkflowContinuePayload>(&envelope, sender)
    else {
        return;
    };
    engine.set_status(WorkflowStatus::Building).await;
