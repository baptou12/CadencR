...
/// neither path applies. The project-branch path returns `Err` to abort the
/// prompt with a user-visible message (a failed `git checkout -b` must not let
/// the agent silently run on the wrong branch); the worktree path degrades to
/// the original cwd internally, as before.
