...
use claude_agent_sdk_rs::{PermissionRequest, PermissionResult};
...
pub async fn resolve_permission_check(
    ...
    match resolved {
        ResolvedPermission::Allow => {
            ...
            ResolvedAction::Resolved(PermissionResult::Allow {
        ...
        ResolvedPermission::Deny { reason } => {
            ...
            ResolvedAction::Resolved(PermissionResult::Deny {
                message: reason,
        ...
        ResolvedPermission::NeedsPrompt { description, pattern } => {
            ...
            if !force_prompt && allowed_patterns.contains(&pattern) {
                ...
                return ResolvedAction::Resolved(PermissionResult::Allow {
                    updated_input: request.input.clone(),
                    updated_permissions: None,
                    tool_use_id: Some(request.tool_use_id.clone()),
                });
