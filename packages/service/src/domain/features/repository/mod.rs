...
mod tests {
    ...
    async fn create_test_project(pool: &SqlitePool) -> i64 {
        let result =
            sqlx::query("INSERT INTO projects (name, path) VALUES ('Test Project', '/tmp/test')")
                .execute(pool)
                .await
                .unwrap();
    ...
    async fn test_list_by_project() {
        ...
        create_feature(&pool, proj1, "Feature A", "ws-feature")
            .await
            .unwrap();
        create_feature(&pool, proj1, "Feature B", "ws-feature")
            .await
            .unwrap();
        create_feature(&pool, proj2, "Feature C", "ws-feature")
            .await
            .unwrap();
    ...
    async fn test_get_by_id() {
        ...
        let id = create_feature(&pool, proj, "My Feature", "ws-feature")
            .await
            .unwrap();
    ...
    async fn test_create_feature() {
        let pool = setup_test_db().await;
        let proj = create_test_project(&pool).await;
        let id = create_feature(&pool, proj, "New Feature", "ws-session")
            .await
            .unwrap();
    ...
    async fn test_get_max_session_num() {
        ...
        create_feature(&pool, proj, "Session 1", "ws-session")
            .await
            .unwrap();
        create_feature(&pool, proj, "Session 3", "ws-session")
            .await
            .unwrap();
        create_feature(&pool, proj, "Not a session", "ws-feature")
            .await
            .unwrap();
    ...
    async fn test_update_title() {
        let pool = setup_test_db().await;
        let proj = create_test_project(&pool).await;
        let id = create_feature(&pool, proj, "Old Title", "ws-feature")
            .await
            .unwrap();
    ...
    async fn test_is_empty_true() {
        let pool = setup_test_db().await;
        let proj = create_test_project(&pool).await;
        let id = create_feature(&pool, proj, "Empty Feature", "ws-feature")
            .await
            .unwrap();
    ...
    async fn test_is_empty_false_has_messages() {
        let pool = setup_test_db().await;
        let proj = create_test_project(&pool).await;
        let id = create_feature(&pool, proj, "Session Feature", "ws-session")
            .await
            .unwrap();
    ...
    async fn test_is_empty_false_has_prd() {
        let pool = setup_test_db().await;
        let proj = create_test_project(&pool).await;
        let id = create_feature(&pool, proj, "Feature With PRD", "ws-feature")
            .await
            .unwrap();
    ...
    async fn test_get_plan_with_phases() {
        let pool = setup_test_db().await;
        let proj = create_test_project(&pool).await;
        let fid = create_feature(&pool, proj, "Feature", "ws-feature")
            .await
            .unwrap();

        let plan_res = sqlx::query(
            "INSERT INTO plans (feature_id, title, status) VALUES (?, 'Plan', 'active')",
        )
        .bind(fid)
        .execute(&pool)
        .await
    ...
    async fn test_reset_phase() {
        ...
        let sess_count: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM agent_sessions WHERE phase_id = ?")
                .bind(phase_id)
                .fetch_one(&pool)
                .await
                .unwrap();
    ...
    async fn test_override_phase_status() {
        ...
        override_phase_status(&pool, phase_id, "completed")
            .await
            .unwrap();
    ...
    async fn test_get_set_feature_settings() {
        ...
        set_feature_setting(&pool, fid, "instructions", "do this")
            .await
            .unwrap();

        // Set a column-based setting
        set_feature_setting(&pool, fid, "model_plan", "claude-3")
            .await
            .unwrap();
    ...
    async fn test_get_set_feature_model_settings() {
        ...
        set_feature_model_setting(&pool, fid, "plan", "claude-3-opus")
            .await
            .unwrap();
        set_feature_model_setting(&pool, fid, "session", "claude-3-haiku")
            .await
            .unwrap();
    ...
    async fn test_resolve_working_dir_fallback_to_project() {
        ...
        let proj_res =
            sqlx::query("INSERT INTO projects (name, path) VALUES ('Proj', '/tmp/proj')")
                .execute(&pool)
                .await
                .unwrap();
    ...
    async fn test_delete_feature_cascade() {
        ...
        let dc_count: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM diff_comments WHERE feature_id = ?")
                .bind(fid)
                .fetch_one(&pool)
                .await
                .unwrap();
    ...
    async fn test_is_empty_false_active_session() {
        ...
        sqlx::query(
            "INSERT INTO agent_sessions (feature_id, title, status) VALUES (?, 'sess', 'running')",
        )
        .bind(fid)
        .execute(&pool)
        .await
    ...
    async fn test_resolve_working_dir_session_type_uses_worktree() {
        ...
        let proj_res =
            sqlx::query("INSERT INTO projects (name, path) VALUES ('Proj', '/tmp/proj')")
                ...
                .unwrap();
        let proj = proj_res.last_insert_rowid();
        let fid = create_feature(&pool, proj, "Session", "ws-session")
            .await
            .unwrap();
    ...
    async fn setup_test_db_with_queue() -> SqlitePool {
        ...
        sqlx::query("ALTER TABLE workflow_queue ADD COLUMN workflow_type TEXT")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("ALTER TABLE workflow_queue ADD COLUMN group_index INTEGER")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("ALTER TABLE workflow_queue ADD COLUMN config TEXT")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("ALTER TABLE workflow_queue ADD COLUMN result TEXT")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query(
            "ALTER TABLE workflow_queue ADD COLUMN created_at TEXT DEFAULT (datetime('now'))",
        )
        .execute(&pool)
        .await
        .unwrap();
        sqlx::query("ALTER TABLE workflow_queue ADD COLUMN started_at TEXT")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("ALTER TABLE workflow_queue ADD COLUMN ended_at TEXT")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("ALTER TABLE workflow_queue ADD COLUMN pid INTEGER")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("ALTER TABLE workflow_queue ADD COLUMN max_retries INTEGER NOT NULL DEFAULT 1")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query("ALTER TABLE workflow_queue ADD COLUMN retry_count INTEGER NOT NULL DEFAULT 0")
            .execute(&pool)
            .await
            .unwrap();
    ...
    async fn test_get_queue_for_feature() {
        ...
        insert_queue_item(&pool, fid, "feature_build", "prd", None, "ready", 0, None)
            .await
            .unwrap();
        insert_queue_item(
            &pool,
    ...
    async fn test_get_queue_for_feature_includes_phase_title() {
        ...
        let plan_id = sqlx::query("INSERT INTO plans (feature_id, title) VALUES (?, 'Test Plan')")
            ...
            .unwrap()
            .last_insert_rowid();
        let phase_id = sqlx::query(
            "INSERT INTO phases (plan_id, step_number, title) VALUES (?, 1, 'Implement auth')",
        )
        .bind(plan_id)
        .execute(&pool)
        .await
    ...
    async fn test_mark_item_completed() {
        ...
        let item_id =
            insert_queue_item(&pool, fid, "feature_build", "prd", None, "running", 0, None)
                .await
                .unwrap();
        mark_item_completed(&pool, item_id, Some(r#"{"ok": true}"#))
            .await
            .unwrap();
    ...
    async fn test_mark_item_error() {
        ...
        let item_id =
            insert_queue_item(&pool, fid, "feature_build", "prd", None, "running", 0, None)
                .await
                .unwrap();
        mark_item_error(&pool, item_id, Some(r#"{"error": "failed"}"#))
            .await
            .unwrap();
    ...
    async fn test_mark_item_skipped() {
        ...
        let item_id =
            insert_queue_item(&pool, fid, "feature_build", "prd", None, "blocked", 0, None)
                .await
                .unwrap();
    ...
    async fn test_insert_dependency_and_unblock() {
        ...
        let item1 = insert_queue_item(&pool, fid, "feature_build", "prd", None, "ready", 0, None)
            .await
            .unwrap();
        let item2 = insert_queue_item(
            &pool,
            fid,
            "feature_build",
            "plan",
    ...
    async fn test_clear_queue_isolates_features() {
        let pool = setup_test_db_with_queue().await;
        let proj = create_test_project(&pool).await;
        let fid1 = create_feature(&pool, proj, "Feature 1", "ws-feature")
            .await
            .unwrap();
        let fid2 = create_feature(&pool, proj, "Feature 2", "ws-feature")
            .await
            .unwrap();

        insert_queue_item(&pool, fid1, "feature_build", "prd", None, "ready", 0, None)
            .await
            .unwrap();
        insert_queue_item(&pool, fid2, "feature_build", "prd", None, "ready", 0, None)
            .await
            .unwrap();
    ...
    async fn test_is_empty_ws_session_paused_no_messages() {
        ...
        sqlx::query(
            "INSERT INTO agent_sessions (feature_id, title, status) VALUES (?, 'sess', 'paused')",
        )
        .bind(fid)
        .execute(&pool)
        .await
        ...
        assert!(
            empty,
            "ws-session with paused session but no messages should be empty"
        );
    }
    ...
    async fn test_set_feature_setting_upsert() {
        ...
        set_feature_setting(&pool, fid, "custom_key", "value1")
            .await
            .unwrap();
        set_feature_setting(&pool, fid, "custom_key", "value2")
            .await
            .unwrap();
    ...
    async fn test_delete_feature_with_workflow_queue() {
        ...
        sqlx::query("DROP TABLE workflow_queue")
            .execute(&pool)
            .await
            .unwrap();
        sqlx::query(
        ...
        .last_insert_rowid();
        let phase_id = sqlx::query(
            "INSERT INTO phases (plan_id, step_number, title) VALUES (?, 1, 'Phase 1')",
        )
        .bind(plan_id)
        .execute(&pool)
        .await
        ...
        let sess_id =
            sqlx::query("INSERT INTO agent_sessions (feature_id, title) VALUES (?, 'sess')")
                .bind(fid)
                .execute(&pool)
                .await
                .unwrap()
    ...
    async fn test_delete_feature_no_related_data() {
        ...
        sqlx::query("PRAGMA foreign_keys = ON")
            ...
            .unwrap();
        let proj = create_test_project(&pool).await;
        let fid = create_feature(&pool, proj, "Empty Feature", "ws-feature")
            .await
            .unwrap();
    ...
    async fn test_upgrade_draft_items_to_ready() {
        ...
        assert_eq!(
            ...
            "non-draft items should not be upgraded"
        );
    }
    ...
    async fn test_clear_queue_removes_draft_items() {
        ...
        assert!(
            items.is_empty(),
            "clear_queue should remove draft items too"
        );
    }
    ...
    async fn test_unblock_does_not_affect_draft_items() {
        ...
        let draft_id = insert_queue_item(
            &pool,
            fid,
            "feature_build",
            "execute",
