...
pub async fn set_item_agent_session(
    pool: &SqlitePool,
    item_id: i64,
    session_id: i64,
) -> Result<(), AppError> {
...
pub async fn get_workflow_type_for_feature(
    pool: &SqlitePool,
    feature_id: i64,
) -> Result<Option<String>, AppError> {
    let val: Option<String> =
        sqlx::query_scalar("SELECT workflow_type FROM workflow_queue WHERE feature_id = ? LIMIT 1")
            .bind(feature_id)
            .fetch_optional(pool)
            .await?;
    Ok(val)
...
pub async fn get_existing_phase_ids(
    pool: &SqlitePool,
    feature_id: i64,
...
pub async fn get_max_order_index_nullable(
    pool: &SqlitePool,
    feature_id: i64,
) -> Result<Option<i64>, AppError> {
    let val: Option<i64> =
        sqlx::query_scalar("SELECT MAX(order_index) FROM workflow_queue WHERE feature_id = ?")
            .bind(feature_id)
            .fetch_one(pool)
            .await?;
    Ok(val)
...
pub async fn get_max_group_index_nullable(
    pool: &SqlitePool,
    feature_id: i64,
) -> Result<Option<i64>, AppError> {
    let val: Option<i64> =
        sqlx::query_scalar("SELECT MAX(group_index) FROM workflow_queue WHERE feature_id = ?")
            .bind(feature_id)
            .fetch_one(pool)
            .await?;
    Ok(val)
