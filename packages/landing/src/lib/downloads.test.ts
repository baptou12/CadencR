...
describe("selectRecommendedDownload", () => {
  ...
  it("keeps asset URLs pinned to the current landing version", () => {
    expect(DOWNLOAD_ASSETS.every((asset) => asset.url.includes("/v0.2.1/"))).toBe(true);
  });
});
