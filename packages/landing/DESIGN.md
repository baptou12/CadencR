...
The ring geometry, the two cuts, and the palette all live in the `@cadencr/brand` package, which `LogoMark.astro` imports directly and which also generates every committed raster — the PNG favicons, `apple-touch-icon`, the manifest icons, `logo.png`, the PNG-entry `favicon.ico`, the served `favicon.svg` (a baked ground/ink/emerald cut), and `og-image.png`. Edit the mark there and run `pnpm brand:install`; never hand-edit a generated asset. `pnpm brand:check` runs on every commit and fails if a committed asset has drifted from the source.
