...
export const projectsRouter = router({
  ...
  getSettings: publicProcedure
    ...
    .query(({ input }) => {
      ...
      const project = db
        .prepare("SELECT branch_prefix, qa_prompt, agent_autonomy, model_plan, model_brainstorm, model_prd, model_execute, model_risk, model_review, model_session, model_qa FROM projects WHERE id = ?")
  ...
  setSetting: publicProcedure
    ...
    .mutation(({ input }) => {
      ...
      const realColumns = new Set([
        "model_plan", "model_brainstorm", "model_prd", "model_execute", "model_risk", "model_review",
  ...
  getModelSettings: publicProcedure
    ...
    .query(({ input }) => {
      const db = getDatabase();
      const row = db
        .prepare("SELECT model_plan, model_brainstorm, model_prd, model_execute, model_risk, model_review, model_session, model_qa FROM projects WHERE id = ?")
