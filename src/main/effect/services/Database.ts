 ...
 * Uses Layer.scoped so that when the runtime disposes, closeDatabase() is called
 * automatically as the finalizer — replacing the manual closeDatabase() call at shutdown.
 */
export const DatabaseLive = Layer.scoped(
  Database,
  Effect.acquireRelease(
    Effect.sync(() => makeService()),
    (_service) => Effect.sync(() => closeDatabase()),
  ),
);
